                ...
                @foreach($users as $user_element)
                    <div class="crud-card">
                        <div class="crud-card-buttons">
                            <button class="edit edit-button" data-endpoint="{{route('users_edit', ['user' => $user_element->id])}}">
                            ...
                            <button class="delete destroy-button" data-endpoint="{{route('users_destroy', ['user' => $user_element->id])}}">
                                <svg  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"> 
                                    <title>Delete</title>
                        ...
                        <div class="crud-card-data">
                            <h3>Nombre: <span>{{$user_element->name}}</span></h3>
                            <h3>Email: <span>{{$user_element->email}}</span></h3>
                            <h3>Creado el: <span>{{$user_element->created_at}}</span></h3>
                            <h3>Actualizado el: <span>{{$user_element->updated_at}}</span></h3>
                        </div>
                    </div>
                @endforeach
            @endisset
...
@section('form')
    ...
    <div class="crud-form-buttons">
        ...
        <div class="button-panel">
            <div class="save">
                <button class="store-button" data-endpoint="{{route('users_store')}}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            ...
            <div class="erase" >
                <button class="create-button" data-endpoint="{{route('users_create')}}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
    ...
    <div class="crud-form-area">
        <form class="admin-form" id="main">
            ...
            <div class="area-section">
                <label>Email</label>
                <input id="mail" name="email" type="email" autocomplete="name" value="{{$user->email ?? ''}}">
            </div>
            <div class="area-section">
                <label>Password</label>
                <input id="pass" name="password" type="password" autocomplete="name">
            </div>
            <div class="area-section">
                <label>Repeat Password</label>
                <input id="repeat-pass" name="password_confirmation" type="password" autocomplete="name">
            </div>
        </form>
        <form id="image">
            <div class="gallery">
