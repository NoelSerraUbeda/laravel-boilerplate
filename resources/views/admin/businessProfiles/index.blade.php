...
@section('table')
    ...
    <div class="crud-table">
        <div class="card-section">
            @isset($businessProfiles)
                @foreach($businessProfiles as $businessProfile_element)
                    <div class="crud-card">
                        <div class="crud-card-buttons">
                            <button class="edit edit-button" data-endpoint="{{route('businessProfiles_edit', ['businessProfile' => $businessProfile_element->id])}}">
                            ...
                            <button class="delete destroy-button" data-endpoint="{{route('businessProfiles_destroy', ['businessProfile' => $businessProfile_element->id])}}">
                                <svg  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"> 
                                    <title>Delete</title>
                        ...
                        <div class="crud-card-data">
                            <h3>Nombre: <span>{{$businessProfile_element->name}}</span></h3>
                            <h3>Email: <span>{{$businessProfile_element->email}}</span></h3>
                            <h3>Creado el: <span>{{$businessProfile_element->created_at}}</span></h3>
                            <h3>Actualizado el: <span>{{$businessProfile_element->updated_at}}</span></h3>
                        </div>
                    </div>
                @endforeach
            @endisset
...
@section('form')
    ...
    <div class="crud-form-buttons">
        ...
        <div class="button-panel">
            <div class="save">
                <button class="store-button" data-endpoint="{{route('businessProfiles_store')}}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            ...
            <div class="erase" >
                <button class="create-button" data-endpoint="{{route('businessProfiles_create')}}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
    ...
    <div class="crud-form-area">
        <form>
            <div class="tab-content active" data-tab="general">
                <input name="id" type="hidden" value="{{$businessProfile->id ?? ''}}">
                <div class="area-section">
                    <label>Comercial Name</label>
                    <input id="name" name="name" type="text" autocomplete="name" value="{{$businessProfile->name ?? ''}}">
                </div>
                <div class="area-section">
                    <label>Email</label>
                    <input id="mail" name="email" type="email" autocomplete="name" value="{{$businessProfile->email ?? ''}}">
                </div>
            </div>
